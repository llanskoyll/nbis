...
***********************************************************************
               ROUTINES:
                        iafis2nist_fingerprints()
                        iafis2nist_fingerprint()
                        iafis2nist_type_9s()
                        iafis2nist_needed()
                        iafis2nist_type_9()
                        iafis2nist_method()
                        iafis2nist_minutia_type()
                        iafis2nist_pattern_class()
                        iafis2nist_ridgecount()
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_fingerprints - Searches an ANSI/NIST file structure for
...
************************************************************************/
int iafis2nist_fingerprints(ANSI_NIST *ansi_nist)
{
   int i, ret;
   ...
   while(i < ansi_nist->num_records){
      ...
      ret = lookup_binary_field_fingerprint(&imgrecord, &imgrecord_i,
                                            i, ansi_nist);
      /* If error. */
      if(ret < 0)
         return(ret);
      ...
      /* if possible. */
      ret = iafis2nist_fingerprint(&nimgrecord, ansi_nist, imgrecord_i);
      /* If error ... */
      if(ret < 0)
         return(ret);
      ...
      if(ret){
         ...
         if(ansi_nist->version < VERSION_0300) {
            if (substitute_ANSI_NIST_item(0, VER_ID-1, 0, 0, "0300",
                                          ansi_nist) < 0) {
	      fprintf(stderr, "ERROR : iafis2nist_fingerprints : "
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_fingerprint - Takes an ANSI/NIST binary field image
...
#cat:              to a Type-14 record.
   ...
   Return Code:
      ...
      Negative   - system error
************************************************************************/
int iafis2nist_fingerprint(RECORD **oimgrecord, ANSI_NIST *ansi_nist,
                           const int imgrecord_i)
{
   ...
   if((imgrecord_i < 1) || (imgrecord_i > ansi_nist->num_records)){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if((imgrecord->type == TYPE_3_ID) || (imgrecord->type == TYPE_5_ID)){
      fprintf(stderr, "WARNING : iafis2nist_fingerprint : "
   ...
   if(!lookup_ANSI_NIST_field(&cmpfield, &cmpfield_i, BIN_CA_ID, imgrecord)){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if(!lookup_ANSI_NIST_field(&impfield, &impfield_i, IMP_ID, imgrecord)){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if((ret = value2field(&nfield, newtype, LEN_ID, "0"))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, IDC_ID, uint_str))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, IMP_ID,
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, ORI_ID,
                              ansi_nist->records[0])){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if((ret = value2field(&nfield, newtype, SRC_ID,
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, DAT_ID,
                              ansi_nist->records[0])){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if((ret = value2field(&nfield, newtype, CD_ID,
                        (char *)field->subfields[0]->items[0]->value))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, HLL_ID,
                        (char *)field->subfields[0]->items[0]->value))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, VLL_ID,
                        (char *)field->subfields[0]->items[0]->value))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, SLC_ID, "2"))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, HPS_ID, uint_str))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, VPS_ID, uint_str))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, TAG_CA_ID, img_comp))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, newtype, BPX_ID, uint_str))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, FGP_ID, imgrecord)){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if((ret = value2field(&nfield, newtype, FGP3_ID,
                        (char *)field->subfields[0]->items[0]->value))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, BIN_IMAGE_ID, imgrecord)){
      fprintf(stderr, "ERROR : iafis2nist_fingerprint : "
   ...
   if((ret = copy_ANSI_NIST_item(&nitem, field->subfields[0]->items[0]))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   /* Allocate new subfield. */
   if((ret = alloc_ANSI_NIST_subfield(&nsubfield))){
   ...
   if((ret = new_ANSI_NIST_field(&nfield, newtype, DAT2_ID))){
      free_ANSI_NIST_subfield(nsubfield);
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
   ...
   if((ret = update_ANSI_NIST_tagged_record_LEN(nimgrecord))){
      free_ANSI_NIST_record(nimgrecord);
      return(ret);
   }
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_type_9s - Searches an ANSI/NIST file structure for
...
#cat:               converts the fields, populating NIST Type-9 fields. 
   ...
   Return Code:
      ...
      Negative   - system error
************************************************************************/
int iafis2nist_type_9s(ANSI_NIST *ansi_nist)
{
   int i, ret;
   RECORD *ntype_9;
   ...
   for(i = 1; i < ansi_nist->num_records; i++){
      ...
      if(ansi_nist->records[i]->type == TYPE_9_ID){
         ...
         if(iafis2nist_needed(ansi_nist->records[i])){
            /* If so, then convert FBI/IAFIS Type-9 to NIST Type-9. */
            if((ret = iafis2nist_type_9(&ntype_9, ansi_nist, i)))
               return(ret);
            /* Insert new Type-9. */
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_needed - Determines if the specified Type-9 record
...
#cat:              routine returns TRUE, otherwise it returns FALSE.
   ...
   Return Code:
      ...
      Negative   - system error
************************************************************************/
int iafis2nist_needed(RECORD *record)
{
   FIELD *mrcfield;
   int mrcfield_i;

   if(record->type != TYPE_9_ID){
      fprintf(stderr, "ERROR : iafis2nist_needed : "
	      "record type = %d : not Type-9", record->type);
      exit(-2);
   }

   if(!lookup_ANSI_NIST_field(&mrcfield, &mrcfield_i, MRC_ID, record))
      return(TRUE);

   /* Otherwise, NIST fields already exist. */
   fprintf(stderr, "WARNING: iafis2nist_needed : "
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_type_9 - Takes a Type-9 record with FBI/IAFIS fields
...
#cat:              in the record.
   ...
   Return Code:
      ...
      Negative   - system error
************************************************************************/
int iafis2nist_type_9(RECORD **otype_9, ANSI_NIST *ansi_nist,
                      const int type_9_i)
{
   int i, j, ret, field_i;
   ...
   if((ret = value2field(&nfield, TYPE_9_ID, FMT_ID, "S"))){
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, FCP_ID, type_9)){
      fprintf(stderr, "ERROR : iafis2nist_type_9 : "
   ...
   if((ret = iafis2nist_method(&nmethod, method))){
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   if((ret = new_ANSI_NIST_field(&nfield, TYPE_9_ID, OFR_ID))){
      free_ANSI_NIST_subfield(nsubfield);
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, FGN_ID, type_9)){
      fprintf(stderr, "ERROR : iafis2nist_type_9 : "
   ...
   if((ret = value2field(&nfield, TYPE_9_ID, FGP2_ID,
                        (char *)field->subfields[0]->items[0]->value))){
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, APC_ID, type_9)){
      /* Create a new subfield with first item set to "T" for Table 8. */
      if((ret = value2subfield(&nsubfield, "T"))){
         free_ANSI_NIST_record(ntype_9);
         return(ret);
      }
      /* Then set primary pattern class to "UN" for Unknown. */
      if((ret = value2item(&nitem, "UN"))){
         free_ANSI_NIST_subfield(nsubfield);
         free_ANSI_NIST_record(ntype_9);
         return(ret);
      }
      ...
      if((ret = new_ANSI_NIST_field(&nfield, TYPE_9_ID, FPC_ID))){
         free_ANSI_NIST_subfield(nsubfield);
         free_ANSI_NIST_record(ntype_9);
         return(ret);
      }
   ...
   /* Otherwise, use pattern classes in APC field. */
      ...
      for(i = 0; i < field->num_subfields; i++){
         ...
         if((ret = iafis2nist_pattern_class(&nclass, class, finger_pos))){
            free_ANSI_NIST_field(nfield);
            free_ANSI_NIST_record(ntype_9);
            return(ret);
         }
         ...
         if(i == 0){
            ...
            if((ret = value2item(&nitem, nclass))){
               free_ANSI_NIST_subfield(nsubfield);
               free_ANSI_NIST_field(nfield);
               free_ANSI_NIST_record(ntype_9);
               return(ret);
   ...
   if(lookup_ANSI_NIST_field(&field, &field_i, CRA_ID, type_9)){
      /* Allocate new CRP field. */
      if((ret = new_ANSI_NIST_field(&nfield, TYPE_9_ID, CRP_ID))){
         free_ANSI_NIST_record(ntype_9);
         return(ret);
      }
   ...
   if(lookup_ANSI_NIST_field(&field, &field_i, DLA_ID, type_9)){
      /* Allocate new DLT field. */
      if((ret = new_ANSI_NIST_field(&nfield, TYPE_9_ID, DLT_ID))){
         free_ANSI_NIST_record(ntype_9);
         return(ret);
      }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, NMN_ID, type_9)){
      fprintf(stderr, "ERROR : iafis2nist_type_9 : "
   ...
   if((ret = value2field(&nfield, TYPE_9_ID, MIN_ID,
                        (char *)field->subfields[0]->items[0]->value))){
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   if((ret = value2field(&nfield, TYPE_9_ID, RDG_ID, "1"))){
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   if(!lookup_ANSI_NIST_field(&field, &field_i, MAT_ID, type_9)){
      fprintf(stderr, "ERROR : iafis2nist_type_9 : "
   ...
   if((ret = new_ANSI_NIST_field(&nfield, TYPE_9_ID, MRC_ID))){
      free_ANSI_NIST_record(ntype_9);
      return(ret);
   }
   ...
   for(i = 0; i < field->num_subfields; i++){
      ...
      if((ret = iafis2nist_minutia_type(&ntype, type))){
         free_ANSI_NIST_subfield(nsubfield);
         free_ANSI_NIST_field(nfield);
         free_ANSI_NIST_record(ntype_9);
         return(ret);
      ...
      /* 5-12. Ridge Counts. */
      while(j < 12){
         if((ret = iafis2nist_ridgecount(&nist_rc,
                                        (char *)subfield->items[j++]->value))){
            free_ANSI_NIST_subfield(nsubfield);
            free_ANSI_NIST_field(nfield);
            free_ANSI_NIST_record(ntype_9);
         ...
         if((ret = value2item(&nitem, nist_rc))){
            free(nist_rc);
            free_ANSI_NIST_subfield(nsubfield);
            free_ANSI_NIST_field(nfield);
            free_ANSI_NIST_record(ntype_9);
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_method - Takes an FBI/IAFIS system method string
#cat:              and returns an ANSI/NIST method.
   ...
   Return Code:
      ...
      Negative  - system error
************************************************************************/
int iafis2nist_method(char **omethod, char *method)
{
   char *nmethod;

   if(strlen(method) != 3){
      fprintf(stderr, "ERROR : iafis2nist_method : "
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_minutia_type - Takes an FBI/IAFIS minutia type
#cat:              and returns an ANSI/NIST minutia type from Table 7.
   ...
   Return Code:
      ...
      Negative  - system error
************************************************************************/
int iafis2nist_minutia_type(char **otype, char *type)
{
   char *ntype;
   ...
   /* Illegal type */
      ...
      fprintf(stderr, "ERROR : iafis2nist_minutia_type : "
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_pattern_class - Takes an FBI/IAFIS pattern classification
...
int iafis2nist_pattern_class(char **oclass, char *class, const int finger_pos)
{
   ...
   /* Illegal pattern class */
      ...
      fprintf(stderr, "ERROR : iafis2nist_pattern_class : "
...
/***********************************************************************
************************************************************************
#cat: iafis2nist_ridgecount - Takes an FBI/IAFIS formatted neighbor/ridge
...
#cat:              by a comma.
   ...
   Return Code:
      ...
      Negative   - system error
************************************************************************/
int iafis2nist_ridgecount(char **onist_rc, char *iafis_rc)
{
   char *nist_rc, c;
   int nbr_i, rc;
